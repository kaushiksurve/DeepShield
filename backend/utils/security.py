...
def validate_video_file(file: UploadFile, content: bytes) -> None:
    ...
    # Be lenient — browsers sometimes report generic content type
    if content_type and content_type not in settings.allowed_mimetypes and content_type != "application/octet-stream":
        raise HTTPException(400, f"MIME type '{content_type}' not accepted")
